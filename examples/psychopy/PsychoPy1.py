#!/usr/bin/env python
# -*- coding: utf-8 -*-
# last mod 2012-09-20 13:38 KS

"""
small example that shows how to use PsyTML from within PsychoPy.
...
data = []       # empty list that will collect the data per trial (as Python
                # dictionnaries)

pathToIdFile = "."      # set the (preferably network) path to the file
...
myWin = visual.Window((1024,768), monitor='testMonitor', fullscr=False,
        allowGUI=False, units ='pix', color = bgColor, winType='pyglet')
...
# initialize screen for PsyTML (the start field is necessary, otherwise the
# first html page will not be on bgColor background when screen size is
# fullscr)
myMouse.setVisible(True)
PsyTML.Start(id, condition)
# if you don't want to present the condition: PsyTML.Start(myWin, myMouse, id)
# I don't understand the sentence above :( cheers Tino/KS
...
# present introduction (if there are more screens, call intro.viewPsyTML()
# multiple times with the correct argument)
intro = PsyTML.PsyTML(size = (900, 800), position = (0, 0)) # in contrast to
        # PsychoPy Position starts in the upper left corner.
# intro = PsyTML.PsyTML(size = (900, 800) # uncomment this line and comment the
        # line above to center the instruction and password.
intro.viewPsyTML("html/intro1.html")

## Present trials
item_win = PsyTML.PsyTML((900, 800)) # create PsyTML object for the items, if
        # position is the default or None it is centered.
for trial_num, item in enumerate(items):
    item_win.viewPsyTML(item)       # present the trial
    data_trial = item_win.data        # read dictionary from trial
    data_trial.update({"id":id, "condition":condition, "trial": trial_num+1})        # here you could add other information that should be written to each trial.
